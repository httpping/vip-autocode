...
import com.baomidou.springboot.test.utils.JdbcTypeNameTranslator;
    ...
    protected ConfigBuilder pretreatmentConfigBuilder(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            for (TableField field : tableInfo.getFields()) {
                //获取真实的jdbc type
                String[] jdbcTypes = field.getType().split("\\(");
                String jdbcType = jdbcTypes[0];
                int jdbc = JdbcTypeNameTranslator.getJdbcType(jdbcType);
                field.setType(JdbcTypeNameTranslator.getJdbcTypeName(jdbc));
            }
            // Boolean类型is前缀处理
